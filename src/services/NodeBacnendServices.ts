...
interface ApiData {
  [key: string]: string | { // api_version, pages
    [key: string]: {  // chart?, common, mesh, status
      [key: string]: Object
    }
  }
}
...
export class NodeBacnendServices implements BackendServices {
  ...
  loadResources(nodeName: string, requiredResources: NodeDataEnum[]): void {
    ...
    if (Object.keys(rb).length === 0) {
      console.log('resources ' + requiredResources.join(',') + ' already loaded')
      return
    }

    const qs = Object.entries(rb).map(([k, v]) => k + '=' + v.join(',')).join('&')
    const url = scheme + nodeName + nodeDomainName + nodePort + '/cgi-bin/api?' + qs
    console.log('loading resources:' + url)

    appStore.setLoading(true)
    axios.get<ApiData>(url).then(({ data }) => {
